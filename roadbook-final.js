...
(() => {
  ...
  const estimatedReturnOdometer = (PICKUP_ODOMETER_KM + TRIP_KM).toLocaleString('en-GB');
  ...
  function route() {
    ...
    return `<section class="wrap">${heading('The itinerary, annotated','The plan met the road.','Two saved planning options. One actual journey, shaped by weather, fatigue, curiosity and the needs of the whole crew.')}
      ...
      <div class="comparison-summary"><article><p class="eyebrow">${esc(plan.label)} · planned</p><strong>${plannedKm.toLocaleString('en-GB')} km</strong><p>${plan.nights} Lofoten nights · arrive ${plan.arrival}<br>Saved road-only stage estimates, through 11 September.</p></article><article class="actual"><p class="eyebrow">What we actually lived</p><strong>${tripDistance} km</strong><p>6 Lofoten nights · arrive 30 August<br>Home on 11 September; final fuel and handover preparations on 12 September.</p></article></div>
      <p class="small-note">The plans total ${plannedKm.toLocaleString('en-GB')} km before the 25 km handover allowance. The actual total is Olivier's corrected whole-trip figure, including local driving; comparison with the planning estimates gives an indicative difference, not a measured detour total. Option B was the saved default.</p>
  ...
  function gaston() {
    ...
    return `<section class="wrap">${heading('The fourth traveller','Gaston. Slightly dramatic.<br>Utterly indispensable.','Our Roadsurfer Cozy Cottage gave us a kitchen, a bed, a refuge from the weather — and a running lesson in patience.')}
      <div class="odometer"><div><p class="eyebrow">Mission completed</p><strong>${tripDistance}<span> km</span></strong><p>Paris → Lofoten → Paris, with everyone safely home.</p></div><div class="odometer-readings"><p><span>Pickup odometer · recorded</span><b>${PICKUP_ODOMETER_KM.toLocaleString('en-GB')} km</b></p><p><span>Return odometer · estimated</span><b>≈${estimatedReturnOdometer} km</b></p><small>${PICKUP_ODOMETER_KM.toLocaleString('en-GB')} + ${tripDistance}, using the corrected trip total. Exact return reading not recorded; excludes any separately unmeasured handover driving.</small></div></div>
