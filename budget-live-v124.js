'use strict';
(function(){
  const DATA_URL='https://raw.githubusercontent.com/opedoussaut/lofoten-2026-data/main/expenses/expenses.json';
  ...
  async function renderLiveBudget(){
    ...
    try{
      ...
      app.innerHTML=`<div class="stack"><section class="card"><p class="eyebrow">BUDGET RÉEL · SOURCE GITHUB</p><h2>Total : ${euro(total)}</h2><p>${expenses.length} dépenses enregistrées. Cette vue lit directement la source de données du roadbook, sans utiliser l'ancien budget embarqué dans le navigateur.</p></section><section class="grid">${cards}</section><section class="card"><div class="table-wrap"><table><thead><tr><th>Date</th><th>Catégorie</th><th>Montant</th><th>Lieu</th><th>Détail</th></tr></thead><tbody>${rows}</tbody></table></div></section></div>`;
    }catch(err){
  ...
  const obs=new MutationObserver(()=>{const b=document.querySelector('#nav button.active[data-tab="budget"]');if(b&&!document.querySelector('[data-live-budget]')){} });
  document.addEventListener('DOMContentLoaded',()=>{const nav=document.getElementById('nav');if(nav)obs.observe(nav,{subtree:true,attributes:true,attributeFilter:['class']});});
})();
